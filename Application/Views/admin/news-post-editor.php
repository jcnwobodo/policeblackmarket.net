    ...
    <div class="row">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
            <h3 class="page-header">
                <span class="glyphicon glyphicon-plus"></span> <?= $data['mode']=='create-post'?"Create":"Update"; ?> News Post
            </h3>
            <div class="text-center mid-margin-bottom <?= $data['status'] ? 'text-success bg-success' : 'text-danger bg-danger';?>"><?= $rc->getFlashData(); ?></div>

            <form method="post" enctype="multipart/form-data" <?= $data['mode']=='update-post'? 'action="'.home_url('/'.$rc->getRequestUrlParam(0).'/update-news-post/?post-id='.$data['post-id'],0).'"':''; ?>>
                ...
                <div class="row">
                    <div class="col-md-8">
                        <div class="form-group form-group-sm">
                            ...
                            <input name="post-title" id="post-title" type="text" class="form-control" value="<?= isset($fields['post-title']) ? $fields['post-title'] : ''; ?>" placeholder="Title of News Post"/>
                        </div>
                        <div class="form-group form-group-sm">
                            <label for="post-url"><span class="glyphicon glyphicon-link"></span> Post URL</label>
                            <div class="input-group">
                                <span class="input-group-addon"><?php home_url('/news/')?></span>
                                <input name="post-url" id="post-url" type="text" class="form-control" value="<?= isset($fields['post-url']) ? $fields['post-url'] : ''; ?>" placeholder="news-post-title"/>
                            </div>
                        </div>
                        <div class="form-group form-group-sm">
